...
The goal of these datasets is to provide users with results from two
surveys, the first asking questions on household water costs and coping
mechanisms, the second characterizing each available water point.
Surveys were conducted from October-November 2023 in two communities in
Accra, Ghana (Korle Gonno and Abuja CBD). The survey was conducted in
partnership with the University of Ghana and with funding from the
Fulbright US Student Program. The data sets are associated with the
following [project
...
water costs and coping strategies in Accra. It has `r nrow(households)`
...
conducted in Accra as well. It has `r nrow(waterpoints)` observations
and `r ncol(waterpoints)` variables. For an overview of the variable
names, see the following table. observations and `r ncol(households)`
variables.
